...
impl<'a> MetaAcc<'a> {
    ...
    #[inline]
    fn extend_user_params_ids(&mut self, tag: TagId, id: u32, parent_id: u32, users: &[UserParam]) {
        let len = users.len();
        self.tags.resize(self.tags.len() + len, tag as u8);
        self.owners.resize(self.owners.len() + len, id);
        self.parents.resize(self.parents.len() + len, parent_id);

        self.out.reserve(len);
        for u in users {
            let combined_value = match &u.value {
                Some(v) => format!("{}\0{}", u.name, v),
                None => format!("{}\0", u.name),
            };

            self.out.push(CvParam {
                cv_ref: None,
                accession: None,
                name: String::new(),
                value: Some(combined_value),
                unit_cv_ref: u.unit_cv_ref.clone(),
                unit_name: u.unit_name.clone(),
                unit_accession: u.unit_accession.clone(),
            });
        }
    }
...
fn build_global_meta_items(
...

    {
        ...
        if !run.user_params.is_empty() {
            meta.extend_user_params_ids(TagId::UserParam, run_id, 0, &run.user_params);
        }
...
pub fn encode(mzml: &MzML, compression_level: u8, f32_compress: bool) -> Vec<u8> {
    ...
    let (container_spect, block_count_spect) = {
        let spect_mode = if compression_level > 0 {
            CompressionMode::Compressed(DefaultCompressor::new(compression_level as i32).unwrap())
        } else {
            CompressionMode::Raw
        };

        let spect_filter = if do_shuffle {
            FilterType::Shuffle
        } else {
            FilterType::None
        };
        ...
        let mut spect_builder =
            ContainerBuilder::new(TARGET_BLOCK_UNCOMP_BYTES, spect_mode, spect_filter);
                ...
                for ba in &bal.binary_data_arrays {
                    if let Some(arr) = array_ref(ba) {
                        ...
                        let (block_id, element_off) = spect_builder
                            .add_item_to_box(arr.len() * esz, esz, |buf| {
    ...
    let (container_chrom, block_count_chrom) = {
        let chrom_mode = if compression_level > 0 {
            CompressionMode::Compressed(DefaultCompressor::new(compression_level as i32).unwrap())
        } else {
            CompressionMode::Raw
        };

        let chrom_filter = if do_shuffle {
            FilterType::Shuffle
        } else {
            FilterType::None
        };
        ...
        let mut chrom_builder =
            ContainerBuilder::new(TARGET_BLOCK_UNCOMP_BYTES, chrom_mode, chrom_filter);
