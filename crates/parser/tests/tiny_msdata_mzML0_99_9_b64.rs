...
const PATH: &str = "data/b64/tiny.msdata.mzML0.99.9.b64";
...
fn tiny_msdata_mzml0_99_9_header_sections() {
    ...
    let p0 = sw0.software_param.first().expect("softwareParam parsed");
    assert_eq!(p0.cv_ref.as_deref(), Some("MS"));
    assert_eq!(p0.accession, "MS:1000533");
    assert_eq!(p0.name, "Bioworks");
    assert_eq!(p0.version.as_deref(), Some("3.3.1 sp1"));

    let sw1 = &sw_list.software[1];
    assert_eq!(sw1.id, "ReAdW");
    let p1 = sw1.software_param.first().expect("softwareParam parsed");
    assert_eq!(p1.cv_ref.as_deref(), Some("MS"));
    assert_eq!(p1.accession, "MS:1000541");
    assert_eq!(p1.name, "ReAdW");
    assert_eq!(p1.version.as_deref(), Some("1"));

    let sw2 = &sw_list.software[2];
    assert_eq!(sw2.id, "Xcalibur");
    let p2 = sw2.software_param.first().expect("softwareParam parsed");
    assert_eq!(p2.cv_ref.as_deref(), Some("MS"));
    assert_eq!(p2.accession, "MS:1000532");
    assert_eq!(p2.name, "Xcalibur");
    assert_eq!(p2.version.as_deref(), Some("2.0.5"));
